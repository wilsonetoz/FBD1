...
# Dependency to get database connection for admin user
def get_admin_db():
    conn = get_db_connection(user="admin_user", password="Vagalinha")
    if not conn:
        raise HTTPException(status_code=500, detail="Could not connect to database as admin.")
    try:
        yield conn
    finally:
        if conn:
...
# Dependency to get database connection for read-only user
def get_readonly_db():
    conn = get_db_connection(user="readonly_user", password="Vagalinha")
    if not conn:
        raise HTTPException(status_code=500, detail="Could not connect to database as read-only user.")
    try:
        yield conn
    finally:
        if conn:
...
# --- Cliente Endpoints (CRUD) ---

@app.post("/clientes/", response_model=Cliente, status_code=status.HTTP_201_CREATED)
async def create_cliente(cliente: ClienteCreate, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
        query = """
...
async def read_clientes(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def read_cliente(cliente_id: int, db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def update_cliente(cliente_id: int, cliente: ClienteCreate, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
        query = """
...
async def delete_cliente(cliente_id: int, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
...
# --- Produto Endpoints (CRUD) ---
@app.post("/produtos/", response_model=Produto, status_code=status.HTTP_201_CREATED)
async def create_produto(produto: ProdutoCreate, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
        query = """
...
async def read_produtos(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def update_produto(produto_id: int, produto: ProdutoCreate, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
        query = """
...
async def delete_produto(produto_id: int, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
...
# --- Pedido Endpoints (CRUD) ---
@app.post("/pedidos/", response_model=Pedido, status_code=status.HTTP_201_CREATED)
async def create_pedido(pedido: PedidoCreate, db: psycopg2.extensions.connection = Depends(get_admin_db)):
    cursor = db.cursor()
    try:
        query = """
...
async def read_pedidos(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def get_detalhes_pedidos_clientes(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def get_produtos_em_promocao(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
...
async def get_total_vendas_por_categoria(db: psycopg2.extensions.connection = Depends(get_readonly_db)):
    cursor = db.cursor()
    try:
        ...
        for row in cursor.fetchall():
            results.append(TotalVendasPorCategoria(
                ...
                Quantidade_Total_Vendida=row[2] # Mapeie para o novo campo
            ))
        return results
    except psycopg2.Error as e:
        raise HTTPException(status_code=500, detail=f"Database error: {e}")
